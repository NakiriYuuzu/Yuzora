...
Supported tours: sample terminal (`help`, `ls`, `git status`, `bun test`, `clear`), creating files/folders, file editing and tabs, Git unified/split diffs and staging sample changes, SQL over agents/sessions/workspaces, theme/accent selection, bilingual UI, Space appearance and sidebar controls. Other native operations are unavailable in the demo; HERDR tools and the full Session window show the desktop-app notice.
