...
describe("worktree inventory reconciliation races", () => {
  ...
  it("preserves execution-origin updates and clears through inventory reprojection", async () => {
    const originSnapshot: HerdrSnapshot = {
      ...snapshot,
      agents: [
        {
          id: "agent-1",
    ...
    expect(useHerdrStore.getState().snapshot?.agents[0]?.executionOrigin).toEqual({
      kind: "wsl",
      distribution: "Ubuntu"
    })
    ...
    expect(
      useHerdrStore.getState().runtimesBySession.default?.baseSnapshot?.agents[0]
        ?.executionOrigin
    ).toEqual({ kind: "wsl", distribution: "Ubuntu" })
    ...
    expect(useHerdrStore.getState().snapshot?.agents[0]?.executionOrigin).toBeUndefined()
