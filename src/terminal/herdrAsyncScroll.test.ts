...
it("coalesces wheel events during initial hydration without polling or inventing history", async () => {
  ...
  await vi.waitFor(() => expect(write.mock.calls).toEqual([[6]]))
  controller.dispose()
})
...
it("dispatches WSL wheel immediately, shares drag position, then reconciles overflow in the background", async () => {
  ...
  const third = deferred<typeof base>()
  const change = vi.fn()
  const write = vi.fn()
  ...
  expect(write.mock.calls).toEqual([[3], [800], [805]])
  first.resolve({ ...base, offsetFromBottom: 3, maxOffsetFromBottom: 2100 })
  second.resolve({ ...base, offsetFromBottom: 800, maxOffsetFromBottom: 2100 })
  ...
  third.resolve({ ...base, offsetFromBottom: 805, maxOffsetFromBottom: 2200 })
