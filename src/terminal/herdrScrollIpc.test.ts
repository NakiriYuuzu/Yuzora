...
it("keeps runtime/pane identity and absolute bottom offset at the typed boundary", async () => {
  ...
  expect(invokeHerdr).toHaveBeenLastCalledWith("herdr_pane_scroll_state", { sessionName: scope, paneId: "w1:p1" }, undefined)
  await setPaneScroll(scope, "w1:p1", 0)
  expect(invokeHerdr).toHaveBeenLastCalledWith("herdr_pane_scroll_to", { sessionName: scope, paneId: "w1:p1", offsetFromBottom: 0 }, undefined)
