...
impl<'a> QueryStream<'a> {
    ...
    fn record_done_count(&mut self, count: Option<u64>) {
        if let Some(count) = count {
            self.rows_affected.push(count);
        }
    }

    fn record_done_token(&mut self, token: &ReceivedToken) {
        let count = match token {
            ReceivedToken::Done(done)
            | ReceivedToken::DoneInProc(done)
            | ReceivedToken::DoneProc(done) => done.rows_if_count_valid(),
            _ => None,
        };
        self.record_done_count(count);
    }

    /// Server-reported valid DONE counts in wire order. This is meaningful
    /// after the stream has been drained to EOF; an empty slice means SQL
    /// Server did not report a count (for example under SET NOCOUNT ON).
        ...
        loop {
            ...
            return match token {
                ...
                | ReceivedToken::DoneProc(done) => {
                    this.record_done_count(done.rows_if_count_valid());
                    continue;
                }
                _ => continue,
            };
...
mod yuzora_tests {
    ...
    #[test]
    fn affected_counts_preserve_wire_order_and_confirmed_zero() {
        let token_stream = futures_util::stream::empty::<crate::Result<ReceivedToken>>().boxed();
        let mut stream = QueryStream::new(token_stream);
        stream.record_done_count(Some(3));
        stream.record_done_count(None);
        stream.record_done_count(Some(0));
        stream.record_done_count(Some(7));
        assert_eq!(stream.rows_affected(), &[3, 0, 7]);
    }
}
