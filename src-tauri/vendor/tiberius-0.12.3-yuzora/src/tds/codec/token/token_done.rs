...
impl TokenDone {
    ...
    /// SQL Server only defines `done_rows` when the DONE_COUNT status bit is
    /// present. A missing bit is "not reported" and must not be fabricated as
    /// a server-confirmed zero.
    pub(crate) fn rows_if_count_valid(&self) -> Option<u64> {
        self.status
...
mod tests {
    ...
    #[test]
    fn count_bit_distinguishes_confirmed_zero_from_missing_count() {
        let confirmed_zero = TokenDone {
            status: DoneStatus::Count.into(),
            cur_cmd: 0,
            done_rows: 0,
        };
        assert_eq!(confirmed_zero.rows_if_count_valid(), Some(0));

        let absent = TokenDone {
            status: BitFlags::empty(),
            cur_cmd: 0,
            done_rows: 99,
        };
        assert_eq!(absent.rows_if_count_valid(), None);
    }
}
