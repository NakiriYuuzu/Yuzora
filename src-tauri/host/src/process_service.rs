...
use crate::{log_event as logging, process_kill};
...
impl ProcessManager {
    ...
    pub fn with_seams(log: LogFn, emit: EmitFn) -> Self {
        Self {
    ...
    pub fn start(
        ...
        let reservation_id = NEXT_RESERVATION_ID.fetch_add(1, Ordering::Relaxed);
        {
            ...
            if self.shutdown.load(Ordering::SeqCst) {
                return Err("dev server manager has shut down".into());
            }
    ...
    /// Permanently close an app-owned lane, including queued blocking starts.
...
fn shell_command(command: &str) -> Command {
    ...
    #[cfg(not(windows))]
    {
        let mut cmd = Command::new(crate::shell::resolve_shell(None));
...
mod tests {
    ...
    #[test]
    fn shutdown_prevents_a_late_start_from_spawning() {
        let manager = Arc::new(ProcessManager::with_parts());
