...
pub use crate::lsp_catalog as catalog;
use crate::lsp_plan as plan;
...
fn sanitize_diagnostic(bytes: &[u8]) -> String {
    ...
    text = crate::log_event::mask_url_userinfo(&text);
...
fn run_command_inner(
    ...
    let deadline = std::time::Instant::now() + timeout;
    loop {
        if std::time::Instant::now() > deadline || crate::cancellation::check().is_err() {
...
fn download_to_file(
    ...
    crate::cancellation::check()?;
    // Run within the install's blocking worker, but keep network I/O async so
    // EOF/cancel can interrupt DNS, TLS, response headers and a stalled body.
    ...
    let result = runtime.block_on(async {
        tokio::select! {
            ...
            result = download_async(url, dest, language, emit) => result,
        }
    });
    // Resolver workers must not turn cancellation back into a blocking wait.
...
fn replace_managed_dir(
    ...
    if let Err(error) = build(&staging).and_then(|_| crate::cancellation::check()) {
...
pub fn install(
    ...
    let lock = options
        ...
        .map_err(|e| e.to_string())?;
    lock.try_lock()
        .map_err(|e| format!("lsp-install-busy: {e}"))?;
    finalize(language, emit, || {
...
mod tests {
    ...
    #[tokio::test]
    async fn cancellation_interrupts_stalled_headers_and_body_and_removes_partial_files() {
        ...
        for send_headers in [false, true] {
            ...
            let dest = temp.path().join("asset.download");
            let worker_dest = dest.clone();
            let cancelled = Arc::new(AtomicBool::new(false));
            let worker_cancel = cancelled.clone();
            let worker = tokio::task::spawn_blocking(move || {
                crate::cancellation::with_cancellation(worker_cancel, || {
                    download_to_file(&url, &worker_dest, "rust", &|_| {})
                })
            });
            ...
            if send_headers {
                socket
                    ...
                    .unwrap();
                tokio::time::timeout(Duration::from_secs(3), async {
                    while std::fs::metadata(&dest).map(|m| m.len()).unwrap_or(0) == 0 {
            ...
            assert!(!dest.exists(), "cancelled download left a partial file");
            assert_eq!(
    ...
    #[test]
    fn cancelled_staged_install_preserves_the_previous_server() {
        use std::sync::atomic::{AtomicBool, Ordering};
        let temp = tempfile::tempdir().unwrap();
        let target = temp.path().join("server");
        std::fs::create_dir(&target).unwrap();
        std::fs::write(target.join("version"), "previous").unwrap();
        let cancelled = Arc::new(AtomicBool::new(false));
        let result = crate::cancellation::with_cancellation(cancelled.clone(), || {
            replace_managed_dir(&target, |staging| {
        ...
        assert!(!managed_sibling(&target, ".installing").exists());
    }

    #[cfg(unix)]
    #[test]
    fn cancelling_install_reaps_the_running_package_manager() {
        use std::sync::atomic::{AtomicBool, Ordering};
        let temp = tempfile::tempdir().unwrap();
        ...
        let job = std::thread::spawn(move || {
            crate::cancellation::with_cancellation(worker_cancel, || {
                run_command(
                    ...
                    &[
                        "-c".into(),
                        "echo $$ > \"$1\"; sleep 30".into(),
                        "fixture".into(),
                        worker_marker.to_string_lossy().into_owned(),
                    ],
