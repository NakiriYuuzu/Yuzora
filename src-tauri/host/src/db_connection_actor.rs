...
impl ProductionConnectionActor {
    pub fn new(identity: ConnectionIdentity, handle: DbHandle) -> Self {
        let sqlite_cancel = match &handle {
            ...
            DbHandle::Postgres(_) | DbHandle::Mssql(_) | DbHandle::RemoteSqlite(_) => None,
        };
        let postgres_cancel = match &handle {
            DbHandle::Postgres(connection) => Some(connection.worker().cancel_handle()),
            DbHandle::Sqlite(_) | DbHandle::Mssql(_) | DbHandle::RemoteSqlite(_) => None,
        };
        Self {
    ...
    pub fn request_lifecycle_teardown(&self) -> Result<LifecycleTeardownRequest, ActorError> {
        if let DbHandle::RemoteSqlite(remote) = &self.handle {
            remote.abort();
        }
        let state = self
            .core
        ...
        let driver_action = if state.busy {
            match &self.handle {
                DbHandle::RemoteSqlite(_) => LifecycleDriverAction::None,
                DbHandle::Sqlite(_) => {
                    self.sqlite_cancel
                        .as_ref()
