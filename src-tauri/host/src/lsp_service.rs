...
use crate::{log_event as logging, lsp_adapters, lsp_config};
...
pub use crate::lsp_framing::{frame, parse_frames, which, which_toolchain};
...
pub type LogFn = Box<dyn Fn(logging::LogEvent) + Send + Sync>;
pub type EmitFn = Box<dyn Fn(LspServerInfo) + Send + Sync>;
...
impl LspManager {
    pub fn with_parts(trace_dir: PathBuf, backoff_ms: u64, log: LogFn, emit: EmitFn) -> Self {
        Self {
    ...
    pub fn with_host_executables(mut self) -> Self {
        self.host_executables = true;
        self
    }
    ...
    pub fn start(
        ...
        let path = match resolve_managed_launch_path(
            language,
            &resolved.server_id,
            &resolved.command,
        ...
        // never spawns; the single-server guarantee holds under concurrency.
        {
            ...
            if self.shutting_down.load(Ordering::Acquire) {
                ...
                info.status = LspProcessStatus::Stopped;
                return info.clone();
            }
            if map.len() >= 128 && !map.contains_key(&key) {
                let mut info = shared.info.lock().unwrap();
                info.status = LspProcessStatus::Crashed {
                    reason: "lsp-server-limit".into(),
                };
                return info.clone();
            }
        ...
        if shared.stopped.load(Ordering::SeqCst) || self.shutting_down.load(Ordering::Acquire) {
    ...
    pub fn send(&self, workspace: &str, language: &str, message: String) -> Result<(), String> {
        ...
        if message.len() > 8 * 1024 * 1024 {
            return Err("lsp-message-too-large".into());
        }
        let bytes = frame(&message);
        {
            ...
            while !remaining.is_empty() {
                ...
                if Instant::now() >= deadline {
                    return Err("lsp-write-timeout".into());
                }
    ...
    pub fn stop_matching(&self, workspace: Option<&str>, language: &str) {
        let keys: Vec<(String, String)> = {
            let map = self.servers.lock().unwrap();
            map.keys()
                .filter(|(w, l)| {
    ...
    fn spawn_child(self: &Arc<Self>, shared: &Arc<ServerShared>) -> Result<(), String> {
        crate::lsp_install_verification::verify_launch(
            &shared.language,
        ...
        if let Some(stdin) = &stdin {
            ...
            {
                let _ = crate::process_kill::kill_tree(&mut child);
                return Err("lsp-nonblocking-stdin-failed".into());
            }
        }
...
fn stderr_loop(shared: Arc<ServerShared>, stderr: std::process::ChildStderr) {
    ...
    loop {
        ...
        if kept.len() < STDERR_KEEP_LINES {
            kept.push(String::from_utf8_lossy(&chunk).trim_end().to_owned());
            let joined = kept.join("\n");
            let mut info = shared.info.lock().unwrap();
            info.last_startup_log = Some(joined.clone());
            info.last_error = Some(joined);
        }
    }
}
...
fn resolve_managed_launch_path(
    ...
    crate::lsp_install_verification::verify_launch(language, server_id, &path, host_executables)
        .map_err(|error| reinstall_hint(&format!("找不到可用的 {command}"), &error))?;
    Ok(path)
}

pub fn detect_server_info_from(
    ...
    detect_server_info_with_policy(cfg, workspace, language, active, resolve_command, false)
}

pub fn detect_host_server(workspace: &str, language: &str) -> Result<LspServerInfo, String> {
    detect_host_server_scope(Some(workspace), language)
}

pub fn detect_host_server_scope(
    workspace: Option<&str>,
    language: &str,
) -> Result<LspServerInfo, String> {
    let cfg = lsp_config::load_from(&lsp_config::config_path());
    detect_server_info_with_policy(&cfg, workspace, language, None, which, true)
}

fn detect_server_info_with_policy(
    cfg: &lsp_config::LspConfig,
    workspace: Option<&str>,
    language: &str,
    active: Option<LspServerInfo>,
    ...
    let (path, status) = match resolve_command(adapter.command) {
        Some(path) => {
            match crate::lsp_install_verification::verify_launch(
                language,
                &server_id,
                &path,
                host_executables,
...
mod tests {
    ...
    fn detect_tampered_managed_npm_install_requests_reinstall() {
        ...
        let catalog = crate::lsp_catalog::embedded_catalog().unwrap();
        let identity = crate::lsp_catalog::resolve_identity(
            catalog,
            "typescript",
        ...
        crate::lsp_catalog::write_provenance(
            catalog,
            "typescript",
            "typescript-language-server",
            std::env::consts::OS,
