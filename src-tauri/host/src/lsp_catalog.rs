...
fn write_managed_launcher(path: &Path, bytes: &[u8]) -> Result<(), String> {
    use std::io::Write;
    // npm's Unix .bin entry is a symlink into the verified package tree.
    // Replace that entry atomically; writing through it corrupts the package.
    let parent = path.parent().ok_or("managed LSP launcher 缺少父目錄")?;
    let mut staged = tempfile::NamedTempFile::new_in(parent)
        .map_err(|e| format!("建立 managed LSP launcher 暫存檔失敗：{e}"))?;
    staged
        .write_all(bytes)
    ...
    staged
        .persist(path)
        .map_err(|e| format!("替換 managed LSP launcher 失敗：{e}"))?;
    Ok(())
}

pub fn write_npm_launcher(installed: &Path, identity: &NpmIdentity) -> Result<(), String> {
    write_managed_launcher(installed, &npm_launcher_bytes(identity, cfg!(windows)))
}

pub fn write_pip_launcher(installed: &Path, identity: &PipIdentity) -> Result<(), String> {
...
mod tests {
    ...
    #[test]
    #[cfg(unix)]
    fn npm_launcher_replaces_bin_symlink_without_modifying_reviewed_module() {
        let (_tmp, installed, identity, module) = npm_content_fixture();
        let original = std::fs::read(&module).unwrap();
        std::fs::remove_file(&installed).unwrap();
        std::os::unix::fs::symlink("../@vtsls/language-server/bin/vtsls.js", &installed).unwrap();
        write_npm_launcher(&installed, &identity).unwrap();
