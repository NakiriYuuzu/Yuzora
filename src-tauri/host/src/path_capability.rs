...
impl PinnedDir {
    /// Open a descendant directory from this pinned descriptor, never following
    /// symlinks. Host helper operations keep the returned descriptor alive.
    #[cfg(unix)]
    pub fn open_subdir(&self, path: &str) -> Result<Self, PathCapabilityError> {
        let mut fd = self.fd.try_clone().map_err(|_| PathCapabilityError::Io)?;
        if !path.is_empty() {
    ...
    #[cfg(unix)]
    pub fn entries(&self) -> Result<PinnedEntries<'_>, String> {
        use std::os::fd::IntoRawFd;
        // openat(".") creates an independent directory cursor. dup() would
        // share the cursor and make subsequent listings appear empty.
        let file = unix_openat(
            self.fd.as_raw_fd(),
            ".",
        ...
        if dir.is_null() {
            ...
            return Err(format!("directory-open-failed: {error}"));
        }
        Ok(PinnedEntries { root: self, dir })
    }

    #[cfg(unix)]
    pub fn mkdir(&self, name: &SafeLeafName) -> Result<(), String> {
        let name = to_cstring(Path::new(name.as_str()))?;
        if unsafe { libc::mkdirat(self.fd.as_raw_fd(), name.as_ptr(), 0o755) } != 0 {
            return Err(format!(
    ...
    /// Rename atomically without replacing any existing file, directory or link.
    #[cfg(unix)]
    pub fn rename_new(
        &self,
        from: &SafeLeafName,
        ...
        let from = to_cstring(Path::new(from.as_str()))?;
        let to = to_cstring(Path::new(to.as_str()))?;
        #[cfg(target_os = "linux")]
        let result = unsafe {
            libc::renameat2(
                self.fd.as_raw_fd(),
        ...
        return Err("atomic-rename-unavailable".into());
    ...
    fn remove_tree_depth(
        ...
        *budget -= 1;
        if self.existing_kind(name)? == Some(NodeKind::Directory) {
            ...
            {
                return Err(format!(
                    "delete-directory-failed-partial: {}",
                    io::Error::last_os_error()
                ));
            }
            Ok(())
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            ...
            let name = match name.to_str() {
                Ok(name) => name,
                Err(_) => return Some(Err("path-not-utf8".into())),
            };
            if matches!(name, "." | "..") {
                continue;
            }
            let leaf = match SafeLeafName::parse(name) {
                Ok(leaf) => leaf,
                Err(error) => return Some(Err(error.into())),
            };
            match self.root.existing_kind(&leaf) {
                Ok(Some(kind)) => return Some(Ok((name.to_owned(), kind))),
                Ok(None) => continue, // concurrent removal, no entry remains
                Err(error) => return Some(Err(error.into())),
            }
        }
    }
...
#[cfg(windows)]
pub fn windows_file_identity(file: &File) -> Result<(u32, u64, bool), PathCapabilityError> {
...
mod win_at {
    ...
    pub fn open_relative_optional(
        ...
        let (access, disposition, options, file_attrs) = match (kind, mode) {
            (RelativeKind::Directory, RelativeMode::CreateNew) => (
                FILE_LIST_DIRECTORY | FILE_TRAVERSE | FILE_READ_ATTRIBUTES | SYNCHRONIZE,
                FILE_CREATE,
                FILE_DIRECTORY_FILE | FILE_OPEN_REPARSE_POINT | FILE_SYNCHRONOUS_IO_NONALERT,
                0,
            ),
            (RelativeKind::Directory, RelativeMode::OpenDelete) => (
    ...
    pub fn rename_relative(
        ...
        let header = std::mem::offset_of!(RenameInformation, name);
        let bytes = (header + name_bytes).max(std::mem::size_of::<RenameInformation>());
        // NtSetInformationFile supports a pinned RootDirectory. The Win32
        // SetFileInformationByHandle wrapper requires an absolute destination.
        // Allocate pointer-aligned storage for the variable-length structure.
        ...
        let status = unsafe {
            (*info).replace = u8::from(replace);
            (*info).root = dest_dir.as_raw_handle();
            (*info).length = name_bytes as u32;
