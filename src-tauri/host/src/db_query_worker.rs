...
pub enum WorkerRequest {
    ConnectPostgres {
        ...
        port: u16,
        #[serde(default)]
        connect_address: Option<std::net::SocketAddr>,
        database: String,
        user: String,
        password: String,
...
pub fn helper_program() -> PathBuf {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(name) = exe.file_name().and_then(|name| name.to_str()) {
            if matches!(
                name,
                "yuzora" | "yuzora.exe" | "yuzora-host" | "yuzora-host.exe"
            ) {
                return exe;
            }
        }
        if let Some(parent) = exe.parent() {
            for name in ["yuzora", "yuzora.exe", "yuzora-host", "yuzora-host.exe"] {
...
impl NetworkQueryWorker {
    ...
    #[allow(
        ...
        reason = "Credentials, transport policy and owned routing stay separate at this shared worker seam"
    )]
    pub async fn spawn_postgres(
        ...
        match worker.handshake(request, "postgres").await {
            Ok(()) if !worker.is_closed() => Ok(worker),
            Ok(()) => {
                worker.reap().await;
                Err(worker_error(
                    "serverDisconnected",
                    "database host disconnected during startup",
                ))
            }
            Err(error) => {
                worker.reap().await;
    ...
    pub fn is_closed(&self) -> bool {
        if self
            .tunnel
            .as_ref()
            .is_some_and(|tunnel| !tunnel.is_current())
        {
            return true;
        }
...
mod tests {
    ...
    async fn declared_oversize_frame_is_rejected_without_reading_the_body() {
        ...
        tokio::spawn(async move {
            ...
            let _ = server.write_all(&[b'x'; 64]).await;
        });
