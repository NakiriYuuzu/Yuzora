...
- 可自訂第 1～9 個頁籤與前後頁籤快捷鍵，支援釘選頁籤、分割區、編輯器、終端機與內建 Browser。
- HTML／HTM 檔案可透過檔案樹、頁籤及編輯器右鍵「在此預覽」，支援本機、WSL 與 SSH 工作區；預覽已儲存內容並在存檔後更新。
- Browser 可選取頁面元素，複製來源、selector、HTML、文字、尺寸與關鍵樣式，方便提供給 AI 修改。

### 改善與修正

- 隨附 HERDR 升級至 0.9.1，保留與 0.9.0 的相容性；升級不主動停止既有 Session。
- 改善終端機在字級調整、分割 pane 與尺寸變更時的內容錯位、閃爍及短暫收縮。
- 修正 Windows 調整終端機字級後，字元間距過大或整行擠在左側的問題。
- 切回應用程式及建立終端機完成命名或取消後，恢復目前 Terminal 的輸入焦點，保留其他輸入欄位與對話框的焦點。
- 新工作區的信任對話框關閉後，恢復原 Terminal 的輸入焦點。
- 修正 Browser 頁籤遮住其他未儲存檔案的關閉確認對話框。
- Git 操作成功與後續狀態刷新失敗分開回報，忙碌時僅重試讀取，避免重複執行提取或分支操作。
- 修正 WSL Windows 磁碟上的檔案已儲存卻顯示失敗的情況，並保留衝突與更新後未儲存內容的提示。

### 使用與安裝說明

- HTML 預覽只讀取已儲存檔案；專案開發伺服器仍由使用者管理。元素複製僅輸出文字，支援同源 iframe 與 open shadow DOM；跨來源 iframe 只選取框架本身，不擷取密碼及即時表單值。
- macOS App 僅支援 Apple Silicon，未使用 Apple Developer ID 簽章或公證；首次開啟仍可能出現 Gatekeeper 提示。
- Windows Authenticode 尚未啟用，安裝或首次開啟時仍可能出現 SmartScreen 提示。
...
- 更新檢查會依 Stable／Preview 頻道讀取對應的簽章發布資訊，並相容於尚未提供新原生更新命令的舊版本。
- WSL 資料夾開啟改用 Explorer 路徑 fallback，確保可在 Windows 檔案總管中開啟。

## [0.0.11] - 2026-09-11

### 修正

- macOS App bundle 改為完整 ad-hoc signing，避免下載後因資源封存不完整而被 Gatekeeper 顯示為已損毀。
- Windows 更新確認視窗會明確提示先關閉 HERDR，避免安裝程式因 `herdr.exe` 被鎖定而失敗。
...
- 外觀設定新增 Bot 動畫開關，可即時停止 Space 夥伴的眨眼、表情、擺動與滑鼠互動位移；低規格裝置預設關閉，並保存手動選擇。
