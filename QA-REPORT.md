...
> 狀態：PR #86 已於 2026-08-28 09:00:58 +08:00 由使用者帳號合併為 main commit `b3c78df04d5f73f24944d6bf2952b1dbbd7db613`；exact-main CI run `33131534173` 7／7 SUCCESS。Release run `33131724866` 的 guard、annotated tag與Windows build成功，但macOS job在任何build前的`Import Developer ID Application certificate`非空檢查失敗：六項Apple signing／notarization secrets全缺，首個錯誤為`APPLE_CERTIFICATE is required for every released macOS installer`。Tag `v0.0.9-beta.1`已由workflow建立並精確指向該main SHA；GitHub Release不存在，assemble／Publish均skipped。這是既知QA-003外部gate的預期fail-closed，不是新的source regression；Windows artifact仍保留於同run。QA-001～QA-046共46個findings，45／45個實際缺陷已完成source修復，QA-019為false positive。Apple secrets與Windows實機仍BLOCKED，整體維持NO-GO。

## 1. 執行摘要

- 測試開始：2026-08-26 00:52:49 +08:00（Asia/Taipei）
- 測試環境：macOS 26.6.1（25G76）、Apple arm64、Bun 1.3.14、rustc/cargo 1.96.0
- 測試來源：原始 QA baseline `d97fb7a5724669394abeb11360eef7330e332d06`；latest remote main `b3c78df04d5f73f24944d6bf2952b1dbbd7db613`；QA-045／QA-046補救PR #86 merged head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`
...
- QA-045 補救 PR exact-head CI／artifact／macOS candidate GUI驗收：2026-08-27 22:08:41 +08:00
- QA-046 補救 PR exact-head CI／artifact／macOS candidate GUI驗收：2026-08-27 22:50:19 +08:00
- PR #86 merge／exact-main CI／Release fail-closed核對：2026-08-28 11:23:39 +08:00
- 整體結論：**NO-GO**
- 功能總數／通過／失敗／阻塞／未測試：**109／92／0／17／0**
- Repository 可處理問題：**45／45 個實際缺陷已完成source修復；QA-046已取得deterministic RED並以顯式poll完整prefix完成最小GREEN，targeted 300／300、完整本機Rust gates、exact-head run `33082371882` 7／7、兩平台候選靜態驗證、bundled Computer Use exact macOS gate與exact-head Final Codex re-review均PASS。run `33078583686`與所有更舊候選維持superseded。QA-019的invalid-ref前提已證偽。QA-003的Developer ID signed／notarized artifact仍受外部憑證阻擋。**
- 上線阻擋問題摘要：
  - QA-003（P1 release gate）：Release run `33131724866`已實證macOS workflow會fail closed；六項Apple secrets全缺，使job在decode／import／build前以`APPLE_CERTIFICATE is required for every released macOS installer`停止。Tag已建立但Release不存在，assemble／Publish均skipped；在取得Developer ID／notary credentials前仍不可發布。
  - QA-045（原P1 release blocker）：PR #85 合併後 exact-main CI run `33075818160` 的 macOS Rust job `98529600496` 曾在 `reader_keeps_partial_bytes_across_short_deadlines` 失敗（890 passed／1 failed／1 ignored）；run `33065472520`及所有較舊候選已superseded。補救PR #86已移除原本固定sleep race；其run `33078583686`後因QA-046 review finding而superseded。
  - QA-046（已解除的P1 release blocker）：Final Codex review thread `PRRT_kwDOTWXJt86c2Ub6`指出PR #86的prefix-ready rendezvous只證明server完成`write_all`，不證明client已consume prefix；受控150ms pre-poll delay穩定RED。commit `97c0780`改為建立短deadline前顯式poll完整prefix；authoritative head `d0e6fdf…`的run `33082371882` 7／7、兩平台候選、macOS GUI gate與Final Codex re-review均PASS，thread已resolved。
  - GitHub secret inventory只有`TAURI_SIGNING_PRIVATE_KEY`與其password；repository／environment secrets與Actions Variables均沒有六項Apple keys。`main`未啟用branch protection且repository rulesets為空；PR #86已由使用者帳號合併，但沒有maintainer approval紀錄。
...
- Authoritative PR #86 exact head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb` 的run `33082371882`七個jobs全部SUCCESS；macOS DMG SHA-256 `9768573d554fc4ef67ff48809945c2a0bc20698eac03724b8dd81d44b250d2a4`、Windows NSIS `5d167bdc9538830d48a5dfda9163c43d3d22563771f313226b7c164c51965af2`、MSI `e8cd8bfc65d5370dda27fa6412ab200a1fdf97e10f74b195852bb6d7c7317b26`。Artifact boundary恰為DMG／NSIS／MSI各一；DMG CRC、`0.0.9-beta.1`／`dev.yuuzu.yuzora`、universal `x86_64 arm64`、預期ad-hoc／no TeamIdentifier、HERDR 0.8.0／protocol 19與exact resources、MSI WiX／`ProductVersion=0.0.2305`／HERDR／ConPTY inventory、NSIS Nullsoft格式均PASS。
- Superseded PR #86 head `52945d08a5cd422eb9997777f72db20cf40e90d8` 的 run `33078583686` 七個 jobs全部成功；macOS DMG SHA-256 `6a9321238f3377ed726047ed496dedfdd292d3c7b7f903b25798e42a1afdb5ae`、Windows NSIS `0d47ab99696e0fbe659ec7fa70b8d27b66e7dc47651ca2edbd231f099db2e2ed`、MSI `07bfde473b7911c20bfd0df40e682547079b3dfd3e226206162091bf0a15a090`。DMG CRC、`0.0.9-beta.1`／`dev.yuuzu.yuzora`、universal `x86_64 arm64`、ad-hoc／no TeamIdentifier預期狀態、HERDR 0.8.0／protocol 19 exact pins、MSI `ProductVersion=0.0.2305`／HERDR／ConPTY inventory與Beta僅DMG／NSIS／MSI的boundary均PASS，但QA-046使整組候選不再具權威性。
- QA-046：在原補救中將short deadline先保存、再受控sleep 150ms，exact targeted test 1／1穩定於`assert!(!pending.is_empty())` RED；修復後client以`poll_local_stream_read`實際consume完整prefix，再建立short deadline。exact targeted 1／1、HERDR transport 9／9、12-way targeted stress 300／300、fmt、all-target check、exact 251-diagnostic Clippy baseline、完整library 891 PASS／1 ignored、SQLite integration 1 PASS／1 ignored與doc tests均PASS；production transport未修改。
...
- Computer Use：run `33036976470` 的完整exact macOS驗收已涵蓋Settings shared Input、Preview physical close、Space／Session／tabs restore與Quit／restart；authoritative run `33082371882`只用bundled Computer Use對exact mounted app path再次確認啟動、Settings `v0.0.9-beta.1`、Tab／Shift+Tab、Escape、兩個Spaces與default Session restore、Space來回切換及Command-Q。最終exact process為0，DMG `/dev/disk4`正常卸載且空mount directory已移除；未使用Orca。
...
- Review：PR #83 的25／25 threads皆已resolved，Final Codex review comment `5434305959`對`a0f7de5708`未發現major issue。PR #85另由Copilot review完整檢查4／4檔且無comments；Final Codex review comment `5438317298`明確檢查`d5e76a0160`並回報「Didn’t find any major issues」。PR #86的Codex review對舊head `52945d08…`新增QA-046；修復後Final Codex re-review comment `5440960750`明確檢查`d0e6fdfcf4`並回報未發現major issue，GraphQL為唯一thread resolved。`reviewDecision`仍空白，因此automated review不取代maintainer approval。
...
| 08 DMG metadata／完整性 | version、identifier、SHA-256、verify | PASS | authoritative run `33082371882` exact-head DMG CRC VALID；`0.0.9-beta.1`／`dev.yuuzu.yuzora`／universal；hash `9768573d…d2a4`。 |
...
| 108 HERDR partial-line deadline regression | macOS full Rust suite、短deadline、partial NDJSON跨poll保存 | PASS | exact-main run `33075818160`／job `98529600496`先取得真實FAIL；受控300ms oversleep deterministic RED。QA-046補強後authoritative run `33082371882` 7／7、兩平台候選與macOS GUI gate均PASS；PR #85 run `33065472520`、PR #86 run `33078583686`及較舊候選維持superseded。 |
| 109 HERDR prefix consumption ordering | server write-ready、client pre-poll deschedule、empty pending | PASS | Final Codex review thread `PRRT_kwDOTWXJt86c2Ub6`指出race；受控150ms pre-poll delay取得`pending.is_empty()` deterministic RED。commit `97c0780`在建立短deadline前顯式poll完整prefix；exact targeted、HERDR 9／9、stress 300／300、完整library 891 PASS／1 ignored、SQLite 1 PASS／1 ignored與所有Rust quality gates均PASS。authoritative run `33082371882` 7／7、兩平台候選靜態驗證、bundled Computer Use macOS gate與Final Codex re-review均PASS。 |
...
- **Bug ID**：QA-003
- **嚴重度**：P1
- **修復狀態**：**WORKFLOW FAIL-CLOSED PASS／RELEASE VERIFICATION BLOCKED** — macOS Stable／Beta皆強制Developer ID signing、notarization、strict `codesign`、`spctl`與app／DMG stapler gate；Release run `33131724866`已證明缺少secrets時會在build前停止，但仍無實際signed candidate可驗。
...
- **錯誤訊息、日誌或畫面證據**：原始unsigned packaged app為`/private/tmp/yuzora-qa-build.mtp9lh/cargo-target/release/bundle/macos/Yuzora.app`。正式attempt：exact-main CI run `33131534173`成功後觸發Release run `33131724866`；macOS job `98722588451`在`Import Developer ID Application certificate`回報`APPLE_CERTIFICATE is required for every released macOS installer`並exit 1。唯讀inventory red loop穩定回報六項全缺；exact-main YAML名稱映射正確，兩個repository Environments均無同名secrets，Actions Variables亦為空，因此排除wrong-name、wrong-scope與certificate-content假設。Workflow已建立annotated tag `v0.0.9-beta.1`並精確指向`b3c78df04d5f73f24944d6bf2952b1dbbd7db613`；Release不存在，assemble／Publish skipped。Windows artifact `9670806546`綁定同run／SHA、未過期，預計2026-09-04 09:20:54 +08:00到期。
- **對上線的影響**：**上線阻擋**。一般使用者從網路取得 app 後會遭 Gatekeeper 信任／啟動阻礙，且無可驗證的發行者 identity。
- **已知暫時解法**：沒有可接受的source workaround；要求使用者繞過Gatekeeper不視為上線方案。由maintainer在GitHub Actions secrets安全提供六項值後，依runbook對同一run執行failed-jobs rerun；不得刪tag、手動上傳macOS artifact或手動Publish以繞過gate。
...
- **前置條件**：Git Log畫面已載入。
- **完整重現步驟**：1. 開啟 Git Log。2. 檢查 User與Date dropdown trigger。3. 比對 shared `Button` composition metadata與互動。
...
- **完整重現步驟**：1. 開啟SSH。2. 點New Host。3. 觀察Password／Key file radio choices尺寸。
...
- **完整重現步驟**：1. 在已有selected Space的Session套用create-only capabilities。2. 建立另一folder Space。3. 讓local workspace open取消或失敗。4. 觀察HERDR focus與Yuzora selection。
...
- **Bug ID**：QA-045
- **嚴重度**：P1（release blocker）
- **修復狀態**：**FIXED／PR #86 HEAD `d0e6fdf…` EXACT-HEAD CI＋CANDIDATE＋MACOS GUI＋FINAL REVIEW PASS；OLDER CANDIDATES SUPERSEDED**
...
- **錯誤訊息、日誌或畫面證據**：CI run `33075818160`、job `98529600496`；失敗test與panic內容如上。PR #85 run `33065472520`與PR #86舊head run `33078583686`雖皆7／7成功，但均已因後續finding標記superseded。補救PR #86 authoritative head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`；本機`cargo fmt --package yuzora -- --check`、`cargo check --locked --all-targets`、exact 251-diagnostic Clippy baseline、`cargo test --locked`及`git diff --check`均PASS；exact-head CI run `33082371882`七個jobs全SUCCESS，DMG／NSIS／MSI hashes、DMG CRC、universal architecture、version／bundle ID、HERDR exact pins、MSI ProductVersion／ConPTY inventory與Beta boundary均PASS，bundled Computer Use exact macOS smoke亦PASS。
- **對上線的影響**：原本main CI失敗使Release workflow依runbook不得建立tag或進入build／publish；PR #86 source修復與exact-head gates現已解除此特定blocker。整體仍因Apple secrets、Windows 11／WSL2實機、maintainer approval與明確merge授權維持NO-GO。
- **已知暫時解法**：source已不需暫時解法；不得沿用run `33065472520`候選、手動建tag或Publish。

### QA-046 — Prefix-ready同步未保證client已consume partial bytes

- **Bug ID**：QA-046
- **嚴重度**：P1（release blocker）
- **修復狀態**：**FIXED IN `97c0780`／EXACT-HEAD CI＋CANDIDATE＋MACOS GUI＋FINAL REVIEW PASS／RUN 33078583686 SUPERSEDED**
- **問題標題**：server完成prefix write的rendezvous仍允許client在第一次poll前oversleep並以空`pending` timeout
- **受影響功能**：HERDR local transport Rust regression gate、QA-045 deterministic remediation、main CI可靠性
- **前置條件**：server已對local stream執行prefix `write_all`並送出`prefix_ready`；client收到signal後，在`read_local_ndjson_line`第一次poll前被排程延遲超過120ms。
- **完整重現步驟**：1. 使用PR #86 head `52945d08…`的既有test。2. 在`prefix_ready_rx.recv()`後、第一次reader call前受控延遲超過120ms。3. reader先執行`deadline_exceeded`，尚未poll socket。4. 觀察TimedOut時`pending`仍為空，`assert!(!pending.is_empty())`失敗。
- **預期結果**：建立短deadline前client必須實際將完整prefix讀入`pending`；後續timeout無論何時被排程都應保留非空partial bytes，resume後組成完整line。
- **實際結果**：原zero-capacity channel只建立「server write返回→client開始」ordering，沒有「client consume完成→建立短deadline」ordering；受控RED如review預測。修復後先以production poll helper將完整prefix讀入`pending`，assert exact bytes後才建立120ms deadline；無論call前是否被排程延遲，TimedOut都保留prefix，resume後可組成完整line。
- **重現率**：Final review source proof 1／1；受控150ms pre-poll deterministic RED 1／1；修復後exact targeted 1／1、12-way stress 300／300、HERDR suite 9／9與完整library 891 PASS／1 ignored。
- **錯誤訊息、日誌或畫面證據**：Final Codex review discussion `r3872564517`／thread `PRRT_kwDOTWXJt86c2Ub6`，reviewed head `52945d08a5cd422eb9997777f72db20cf40e90d8`；RED在`src/herdr_transport.rs:526`得到`assertion failed: !pending.is_empty()`，server隨test unwind收到`RecvError`。修復commit `97c0780`；fmt、all-target check、exact 251-diagnostic Clippy baseline、full Rust／SQLite／doc tests及`git diff --check`均PASS。Authoritative head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`的run `33082371882` 7／7 SUCCESS；DMG `9768573d…d2a4`、NSIS `5d167bdc…65af2`、MSI `e8cd8bfc…17b26`靜態驗證及bundled Computer Use exact macOS gate均PASS。Final Codex re-review comment `5440960750`明確檢查`d0e6fdfcf4`且未發現major issue；thread已回覆並resolved。
- **對上線的影響**：原QA-045 remediation在繁忙runner仍可能失敗，故run `33078583686`不再是最終release evidence。source blocker已於`97c0780`解除，新的exact-head CI、兩平台候選、macOS GUI與Final Codex review gates均已完成；整體release仍受Apple secrets、Windows實機、maintainer approval與明確merge授權阻擋。
- **已知暫時解法**：source不需暫時解法；不得沿用舊candidate，僅可使用run `33082371882`綁定`d0e6fdf…`的候選證據。
...
- 最新本機核對時間：2026-08-28 11:23:39 +08:00；本機仍在`release/v0.0.9-beta.1` @ merged PR #86 head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`，live remote main為使用者合併commit `b3c78df04d5f73f24944d6bf2952b1dbbd7db613`。本輪開始與更新後`git status --short`都只含`M QA-REPORT.md`，staging area保持空白；沒有source／workflow變更。Release run `33131724866`失敗是QA-003外部secret gate，非新source regression。
- 最新 downloaded exact candidates位於 repository外 `/private/tmp/yuzora-rc-33082371882.tqjw3J`。macOS candidate已最終Command-Q、matching process不存在、DMG `/dev/disk4`已正常卸載且空mount directory已移除；本輪驗證未寫 repository `dist/` 或 `src-tauri/target/`。
...
- run `33082371882` packaged Yuzora已以exact app path完成launch／Settings版本／keyboard／restore／Space switch／最終Quit；matching process不存在且DMG已卸載。
...
- 2026-08-27 22:08:41 +08:00：補救PR #86 authoritative head `52945d08a5cd422eb9997777f72db20cf40e90d8`的exact-head CI run `33078583686` frontend、三平台Rust、real database integration與macOS／Windows candidates 7／7 SUCCESS。DMG／NSIS／MSI SHA-256依序為`6a9321238f3377ed726047ed496dedfdd292d3c7b7f903b25798e42a1afdb5ae`、`0d47ab99696e0fbe659ec7fa70b8d27b66e7dc47651ca2edbd231f099db2e2ed`、`07bfde473b7911c20bfd0df40e682547079b3dfd3e226206162091bf0a15a090`。DMG CRC、version／bundle ID、universal architecture、ad-hoc／no TeamIdentifier預期狀態、HERDR 0.8.0／protocol 19 pins、MSI ProductVersion `0.0.2305`／HERDR／ConPTY inventory與Beta no-updater boundary均PASS。
- 2026-08-27 22:09:46 +08:00：只用bundled Computer Use驗收run `33078583686` exact mounted macOS app；launch、Settings版本、Tab／Shift+Tab、Escape、兩個Spaces／default Session restore、Space來回切換與Command-Q均PASS。exact process為0、DMG `/dev/disk4`正常卸載，未使用Orca。Final Codex review已由comment `5440359809`對exact head觸發且GitHub已加EYES反應，結果仍等待完成；PR為0個review threads。
- 2026-08-27 22:14:22 +08:00：Final Codex review對exact head `52945d08…`新增P1 thread `PRRT_kwDOTWXJt86c2Ub6`（discussion `r3872564517`），記錄QA-046並立即把run `33078583686`與其兩平台候選標記superseded。finding證明prefix-ready rendezvous只保證server write完成，client仍可能在第一次poll前oversleep使TimedOut保留空`pending`；未resolve thread、未沿用候選，重新進入deterministic RED→GREEN remediation。
- 2026-08-27 22:16–22:25 +08:00：QA-046先將short deadline保存後受控sleep 150ms，exact test穩定在`assert!(!pending.is_empty())` RED；最小修復在建立short deadline前以`poll_local_stream_read`顯式consume並核對完整prefix，production transport零變更。修復後exact targeted 1／1、12-way stress 300／300、HERDR transport 9／9、fmt、all-target check、exact 251-diagnostic Clippy baseline、完整library 891 PASS／1 ignored、SQLite integration 1 PASS／1 ignored與doc tests均PASS。test-only修復形成commit `97c0780`；新exact-head remote gates待報告commit與branch push後執行。
- 2026-08-27 22:45–22:50 +08:00：PR #86 authoritative head `d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`的exact-head CI run `33082371882` frontend、三平台Rust、real database integration與macOS／Windows candidates 7／7 SUCCESS。DMG／NSIS／MSI SHA-256依序為`9768573d554fc4ef67ff48809945c2a0bc20698eac03724b8dd81d44b250d2a4`、`5d167bdc9538830d48a5dfda9163c43d3d22563771f313226b7c164c51965af2`、`e8cd8bfc65d5370dda27fa6412ab200a1fdf97e10f74b195852bb6d7c7317b26`；DMG CRC、version／bundle ID、universal architecture、預期ad-hoc／no TeamIdentifier、HERDR 0.8.0／protocol 19 exact resources、MSI WiX／ProductVersion `0.0.2305`／HERDR／ConPTY inventory、NSIS Nullsoft格式與Beta artifact boundary均PASS。
- 2026-08-27 22:50:19 +08:00：只用bundled Computer Use驗收run `33082371882` exact mounted macOS app；launch、Settings `v0.0.9-beta.1`、Tab／Shift+Tab、Escape、兩個Spaces／default Session restore、Space切換與Command-Q均PASS。exact process為0、DMG `/dev/disk4`正常卸載且空mount directory已移除，未使用Orca。QA-046 thread已回覆並resolved；exact-head final Codex re-review待觸發。
- 2026-08-27 22:56:07 +08:00：Final Codex re-review comment `5440960750`完成，明確標示reviewed commit `d0e6fdfcf4`並回報未發現major issue；未新增thread。GraphQL讀回PR #86 authoritative full head仍為`d0e6fdfcf434b545a37e0ac1d7040d5ee2690ffb`、唯一thread resolved、PR OPEN／MERGEABLE，但`reviewDecision`仍空白，故只標記automated review PASS，不推定maintainer approval。
- 2026-08-27 22:59:08 +08:00：最終證據已更新至PR #86 comments `5440954827`／`5440987102`與Issue #84 comments `5440954833`／`5440986822`。唯讀一致性檢查確認PR head未漂移、8個visible checks全SUCCESS、run `33082371882`綁定exact head且7／7、唯一thread resolved；GitHub仍只有2項updater secrets、六項Apple secrets全缺、branch protection API 404、rulesets空、Windows 11／WSL2實機證據／maintainer approval／明確merge授權仍缺，tag／Release不存在。Yuzora process與DMG mount無殘留；`git diff --check` PASS，workspace只含`M QA-REPORT.md`且staging area空白。Heartbeat已更新至目前authoritative gate；未merge、未建tag、未Publish。
- 2026-08-28 09:00:58–09:21:44 +08:00：PR #86由`NakiriYuuzu`使用者帳號合併為main commit `b3c78df04d5f73f24944d6bf2952b1dbbd7db613`；本agent未執行merge。Exact-main CI run `33131534173` 7／7 SUCCESS。Release run `33131724866` guard與annotated tag建立成功，Windows build成功；macOS job `98722588451`在任何build前因`APPLE_CERTIFICATE`空值fail closed，assemble／beta Publish均skipped。Tag `v0.0.9-beta.1`精確指向main SHA，GitHub Release不存在；Windows artifact `9670806546`仍可供同run failed-jobs recovery使用。
- 2026-08-28 11:23:39 +08:00：tight read-only secret-inventory loop回報六項Apple keys全缺（6／6）；exact-main workflow mapping逐項一致，repository Environments `copilot`／`github-pages`均無secrets，Actions Variables為空，排除wrong-name／wrong-scope／corrupt-content假設。根因確認為既知QA-003 external provisioning blocker，沒有repository修復可做。安全恢復需maintainer自行在GitHub secret store提供六項值，再對run `33131724866`執行failed-jobs rerun；不得分享secret內容給agent、刪除exact tag、繞過signing或手動Publish。工作區只有`M QA-REPORT.md`且staging area空白。
