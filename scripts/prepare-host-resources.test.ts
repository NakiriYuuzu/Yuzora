...
it("pins four Unix targets, helper paths, and the official HERDR hashes", () => {
  ...
  for (const target of Object.keys(HOST_TARGETS) as Array<keyof typeof HOST_TARGETS>) {
    const artifact: HostArtifact = { protocol:1, version:"0.0.9-beta.3", target, helper:{path:`${target}/yuzora-host`,sha256:"a".repeat(64)}, herdr:{path:`${target}/herdr`,sha256:HERDR_RESOURCE_TARGETS[target].files[0].sha256,version:"0.9.1",protocol: 22} }
